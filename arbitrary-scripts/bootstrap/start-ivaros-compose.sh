...
if [ -n "$USE_DOCKER_SWARM" ]; then  # not tested, leave for future use
  ...
  case $HOST_ARCH in
    arm*|aarch64)
      docker compose --profile ivaros --profile arm -f docker-compose.yml up --build -d --remove-orphans "$@"
      ;;
    x86_64|amd64|x86)
      docker compose --profile ivaros --profile x86_64 -f docker-compose.yml up --build -d --remove-orphans "$@"
      ;;
    ppc64le|ppc64)
      docker compose --profile ivaros --profile ppc64le -f docker-compose.yml up --build -d --remove-orphans "$@"
      ;;
    riscv64)
      docker compose --profile ivaros --profile riscv64 -f docker-compose.yml up --build -d --remove-orphans "$@"
      ;;
    *)
