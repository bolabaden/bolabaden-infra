...
## LFG Rerun Evidence (2026-06-03, strict-order)

### Gate 1: Baseline Health

- `docker compose ps aiostreams traefik stremio stremthru` showed all target services healthy.

### Gate 2: Addon Manifest
...
### Gate 3: Runtime Env Migration

Present in running container:
...
Absent deprecated keys in running container:

- `MAX_STREAM_EXPRESSION_FILTERS`
- `ALLOWED_REGEX_PATTERNS*`
- `MEDIAFUSION_API_PASSWORD_FILE`
- legacy per-service `DEFAULT_<SVC>_*` / `FORCED_<SVC>_*` credential variables

### Gate 4: DB Persistence

`metadata.animeDb.refresh.*` persisted values remain corrected in seconds:

- `extendedAnitraktMovies = 86400`
- `extendedAnitraktTv = 86400`
- `fribbMappings = 86400`
- `kitsuImdbMapping = 86400`
- `manamiDb = 604800`

### Gate 5: Restart Stability

- `docker compose restart aiostreams` completed successfully.
- Post-restart checks:
  - `aiostreams` returned to `Up (...) (healthy)`
  - addon manifest remained `HTTP/2 200` with unchanged `id/name/version`.

## LFG Rerun Acceptance Matrix

- Services healthy before and after restart: PASS
- Addon manifest endpoint health and payload validity: PASS
- v2.30 runtime key presence / deprecated key absence: PASS
- Persisted DB interval unit correctness: PASS
- Overall rerun closure status: PASS
